...
<script lang="ts">
    ...
    async function onStartCam({inverseUseSystemCamera = false, QRCodeOnly = false} : {inverseUseSystemCamera: boolean; QRCodeOnly:boolean}) {
        try {
        ...
        {#if showActionButton}
            <stacklayout bind:this={fabHolder} horizontalAlignment="right" marginBottom={$windowInset.bottom + 16} orientation="horizontal" row={1} verticalAlignment="bottom">
                ...
                {#if $hasCamera}
                    ...
                    text="mdi-camera" 
                    verticalAlignment="center" 
                    on:tap={throttle(() => onStartCam({inverseUseSystemCamera: false, QRCodeOnly: false}), 500)} 
                    on:longPress={() => onStartCam({inverseUseSystemCamera: true, QRCodeOnly: false})} />
                {/if}
                {#if $hasCamera}
                    <mdbutton
                    id="fab-qr"
                    ...
                    verticalAlignment="center"
                    on:tap={throttle(() => onStartCam({inverseUseSystemCamera: false, QRCodeOnly: true}), 500)} />
                {/if}
