...
export class OCRDocument extends Observable implements Document {
    ...
    get isQRCodeOnly() {
        if (typeof this.QRCodeOnly === 'string') {
            return this.QRCodeOnly === "true"
        } else {
            return this.QRCodeOnly
        }
    }
    ...
    static async createDocument(pagesData?: PageData[], QRCodeOnly = false, setRaw = false) {
        const date = dayjs();
        ...
        const doc = await documentsService.documentRepository.createDocument({ id: docId, name, QRCodeOnly } as any);
